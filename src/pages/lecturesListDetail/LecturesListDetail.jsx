...
const LecturesListDetail = () => {
  ...
  return (
    <>
      ...
      {querieddata && (
        <>
          <title>{`${lectureTitleExtractor(querieddata[0]?.title, 2) || "Album"} - ${querieddata[0]?.rp_name || 'Dawahnigeria'}`}</title>
          <meta name="description" content={`Listen to the album ${querieddata[0]?.title || 'Unknown Album'} by ${querieddata[0]?.rp_name || 'various scholars'} on Dawahnigeria. Explore Islamic lectures and resources.`} />
        </>
      )}
      <Container>
